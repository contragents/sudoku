...
menuIcons.forEach(menuIcon => {
    menuIcon.addEventListener('click', e => {
        e.preventDefault();
        if ((menuIcon.dataset.openBlock).includes('catalog-sub-') && window.innerWidth >= 768) return;
        let selectedBlock = document.querySelector(`#${menuIcon.dataset.openBlock}`);
        selectedBlock.classList.toggle(`_opened`);
        html.classList.toggle('lock');
    });
});
;// CONCATENATED MODULE: ./node_modules/fb24m-pro-plus/popup.js


let popups = document.querySelectorAll('[data-popup]');
let popupButtons = document.querySelectorAll('[data-popup-toggle]');

popupButtons.forEach(popupButton => {
    popupButton.addEventListener('click', e => {
        popups.forEach(popup => {
            if (popup.getAttribute('data-popup') == popupButton.getAttribute('data-popup-toggle')) {
                popup.classList.toggle('_popup-opened');
            }
        });
    });
});
...
const $1 = (selector, func) => {
	// Если есть объект по селектору, вызываем функцию с ним
	if (document.querySelector(selector)) func(document.querySelector(selector));
}
...
$1('#calculator', (calculator) => {
	calculator.addEventListener('submit', (e) => {
		...
		$1('#count', (input) => {
		    if(input.dataset.factor == 0){
                price = 0;

		        return;
            }

			count = input.value * input.dataset.factor;
            price = count;
		...
		if (!isNaN(count)) {
			$1('.welcome__total', (total) => {
				total.classList.remove('welcome__total_hidden');
			});

            $1('.welcome__total-price', (priceLabel) => {
                priceLabel.innerHTML = new Intl.NumberFormat("ru", {style: "currency", currency: "RUB"}).format(count);
            });

            $1('.welcome__total-difference', (diffLabel) => {
                diffLabel.innerHTML = 'Разница ' + new Intl.NumberFormat("ru", {style: "percent", minimumFractionDigits: 2}).format((price-count)/price);
            });
		}
	});
});

$1('#application-form', (form) => {
    ...
    const warning = form.querySelector('.popup__warning');

	const inputs = form.querySelectorAll('.input');

	const requestCodeButton = form.querySelector('#request-code');
	const checkCodeButton = form.querySelector('#submit-code');

	const submitButton = form.querySelector('[type="submit"]');

	requestCodeButton.addEventListener('click', (e) => { /* Отправка кода по Email */ });
	checkCodeButton.addEventListener('click', (e) => { /* Проверка кода Email */ });

	inputs.forEach((input) => {
		input.addEventListener('change', (e) => {
			if (phone.value !== '' || email.value !== '') {
				submitButton.disabled = false;
                warning.setAttribute('style', 'color:#333;');
                warning.innerHTML='<strong>Нажимая кнопку Отправить заявку, Вы соглашаетесь с условиями <a href="/?action=user_agreement" target="_blank">Пользовательского соглашения</a>, а также на <a href="/?action=policy" target="_blank">обработку персональных данных</a></strong>';
			}
			else {
                ...
                warning.innerHTML='Не заполненно обязательное поле Телефон ИЛИ Email';
			}
		});
	});
	form.addEventListener('submit', (e) => {
		// Отправка формы на сервер (только если заполнена)
        e.preventDefault();
        let formData = {};

        for (let i = 0; i < inputs.length; i++) {
            const input = inputs[i];
                formData[input.name !== '#' ? input.name : 'message'] = input.value;
        }

        $1('#count', (input) => {
            formData['shares_count'] = input.value;
            formData['share_price'] = input.dataset.factor;
        });

        $1('.welcome__total-price', (priceLabel) => {
            formData['total_cost'] = priceLabel.innerHTML.replace(/&nbsp;/g, '');
        });

        $1('.welcome__total-difference', (diffLabel) => {
            formData['comission'] = diffLabel.innerHTML.replace(/&nbsp;/g, '');
        });

        formData['action'] = 'request';

        fetchGlobal('', 'form_data', formData)
            .then((data) => {

                let closeButton = document.getElementById("order-close");
                closeButton.click();
...
let maskedInputs = document.querySelectorAll('.phone-mask');
...
$1('#request-call', (form) => {
	...
	form.addEventListener('submit', (e) => {
		...
		formData['action'] = 'requestCall';

        fetchGlobal('', 'form_data', formData)
            .then((data) => {

                let closeButton = document.getElementById("request-call-close");
                closeButton.click();
...
// Список страниц
// import "./modules/multypage-list.js";

// Подключение других модулей

/* ==================== */

// Подключение основного скрипта для разработки
