...
function () {
    ...
    const progressWidth = 320 * LOADING_TEXT.length / 'Sudoku is loading...'.length + 60;
    progressBox.fillStyle(0x222222, 0.3);
    progressBox.fillRect(gameWidth / 2 - progressWidth / 2, gameHeight / 2 - 50 / 5, progressWidth, 50);

    const textWidth = this.cameras.main.width;
    const textHeight = this.cameras.main.height;
    ...
    this.load.on('progress', function (value) {
        ...
        progressBar.fillRect(gameWidth / 2 + 10 - progressWidth / 2, gameHeight / 2 - 30 / 2 + 15, progressWidth * value - 20, 30);
    });
    ...
    playerBlockModes.forEach(mode => {
        for (let k in digits.playerDigits[mode]) {
            this.load.svg(mode + '_' + 'player_' + k, 'img/' + mode.toLowerCase() + '/' + digits.playerDigits[mode][k]['filename'] + '.svg',
                {'height': digitHeight(), 'width': digitWidth()}
            );
