...
class Command
{
    ...
    // Специфические методы для наследников | Begin
    public static function getAllReklTeasers($reklID)
    {
        return array_column(\TeaserModel::findAllCustom(['user_id', 'is_deleted'], [$reklID, 0], true, ['id']), 'id');
    }

    public static function getDateTo(): string
    {
        // Собираем запрос на получение максимальной отметки времени изменения/создания настроек Тизеров и РК и Площадок
        $query = 'SELECT max(mx) as mx FROM ( '
            . ORM::select(['max(updated_at) as mx'], \TeaserModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \TeaserModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \TeaserModel::DEVICE_TYPE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \TeaserModel::DEVICE_TYPE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \TeaserModel::GEO_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \TeaserModel::GEO_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \AdvCampaignModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \AdvCampaignModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \AdvCampaignModel::DEVICE_TYPE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \AdvCampaignModel::DEVICE_TYPE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \AdvCampaignModel::GEO_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \AdvCampaignModel::GEO_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \SiteModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \SiteModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \SiteBlockModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \SiteBlockModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \SiteBlockModel::BLOCK_TEMPLATE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \SiteBlockModel::BLOCK_TEMPLATE_TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \BlModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \BlModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \WlModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \WlModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(updated_at) as mx'], \SiteBlModel::TABLE_NAME)
            . ORM::union()
            . ORM::select(['max(created_at) as mx'], \SiteBlModel::TABLE_NAME)
            . ' ) maxes_table';

        return DB::queryValue($query) ?: '';
    }

    public static function saveDateToCache($dateTo): void
    {
        Cache::set(self::LAST_UPDATE_TIMESTAMP_KEY, $dateTo);
    }
    // Специфические методы для наследников | End
