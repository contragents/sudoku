...
class ArrayRedisCache extends ApcuCache implements CacheInterface
{
    static $cacheArr = [];
    static $chunks = [];
    ...
    {
        if (isset(self::$cacheArr[$id])) {
            return self::$cacheArr[$id];
        }

        $keyDbData = Cache::get($id);
        if ($keyDbData || is_array($keyDbData) || $keyDbData === false) {
            self::$cacheArr[$id] = $keyDbData;
    ...
    {
        return isset(self::$cacheArr[$id]) ?: Cache::exists($id);
    }
    ...
    {
        return self::set($id, $data, $lifeTime);
    }
